...
import httpx
from openai import BadRequestError, OpenAI, OpenAIError, DefaultAsyncHttpxClient, \
    AsyncOpenAI
...
class WeeklyOpenAiHoroscope(Horoscope):
    ...
    async def _create_image(
        ...
        *,
        ...
        if improve_prompt:
            improvement_message = await self._improve_image_prompt(messages) or messages[-1]
        else:
